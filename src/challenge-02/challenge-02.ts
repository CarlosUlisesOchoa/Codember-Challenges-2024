...
function charIsANumber(charOrCharCode: string | number): boolean {
  if (typeof charOrCharCode === 'string') {
    // Ensure it's a single character
    if (charOrCharCode.length !== 1) return false
...
function isGeneratedByOmega(str: string): boolean {
  const regEx = /^[a-z0-9]+$/
...
export function getTrueAndFalseOmegaPasswords(passwords: string[]): string {
  //
